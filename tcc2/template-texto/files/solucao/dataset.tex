...
A base de imagens oriunda do site IMDb foi organizada por Rothe et al. considerando uma tarefa de aprendizado análoga à deste trabalho, conforme mencionado na Seção  \ref{sec:trab_relac} \cite{rothe2015dex}. Neste contexto, cada exemplo do conjunto de dados é referente a uma imagem, cujos meta-dados estão descritos em seus atributos, conforme ilustrado na \ref{tab:um_deniro}.

Nesta base, destacam-se as coordenadas da localização de um rosto detectado na imagem, juntamente de uma pontuação produzida pelo detector de faces, que quantifica o grau de certeza na detecção do rosto. Partindo da possibilidade de haver mais de um rosto por imagem, uma segunda pontuação é atribuída pelo detector, referente ao grau de certeza de que há outro rosto na mesma imagem. Neste contexto, os meta-dados pontuação da primeira face e localização da face são referentes ao rosto com maior grau de certeza, enquanto a pontuação da segunda face trata do segundo rosto com maior grau.

Assim, é possível encontrar exemplos do conjunto de dados com especificações difusas. Há exemplos de imagens em que há apenas um rosto, a pontuação da segunda face é nula e a face localizada é a mesma da celebridade referenciada nos meta-dados da imagem, como mostrado na Figura \ref{tab:um_deniro}. Já na Figura \ref{tab:dois_deniro_correto} está um exemplo de imagem onde há mais de um rosto, em que o rosto com maior grau de certeza é aquele pertencente à celebridade referida nos meta-dados da imagem. Por fim, na Figura \ref{tab:dois_deniro_errado} há uma imagem com mais de um rosto, em que o rosto apontado no meta-dado \emph{Localização da Face} por ter apresentado maior nitidez não pertence à celebridade cujos dados estão referenciados.
